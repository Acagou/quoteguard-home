...
import { AlertTriangle, FileUp, Loader2, ScanLine, Settings, ShieldAlert } from "lucide-react";
import { ReportView } from "@/components/ReportView";
import { QuoteReport, SensitiveRepair } from "@/lib/report";
import { AiProvider, getStoredAiSettings } from "@/components/AiSettingsForm";
          ...
          <section className="rounded-lg border border-oat bg-paper p-4 shadow-soft md:p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h2 className="text-2xl font-bold text-ink">{getProviderLabel(aiProvider)}</h2>
                <p className="mt-1 text-sm leading-6 text-slateWarm">
                  {aiProvider === "fallback" ? "Using the built-in local scoring fallback." : `Using saved ${getProviderLabel(aiProvider)} settings on this device.`}
                </p>
              </div>
              <Link href="/settings" className="inline-flex items-center justify-center gap-2 rounded-md bg-night px-4 py-3 text-sm font-bold text-white">
